...
features$is_pfam_candidate <- !is.na(features$isoform_switch_q_value) &
...
candidate_table <- features[features$is_pfam_candidate | features$is_target_gene, , drop = FALSE]
...
output_prefix <- "isoform_pfam_candidates"
...
notes <- c(
    ...
    sprintf("Significant switch candidates in feature table: %d", sum(features$is_pfam_candidate, na.rm = TRUE)),
    ...
    "",
    "Outputs:",
    "- isoform_pfam_candidates_AA.fasta: amino-acid FASTA for pfam_scan.pl/HMMER.",
    "- pfam_top_candidates.csv: compact table of strongest isoforms.",
...
writeLines(notes, file.path(outdir, "pfam_prepare_notes.txt"))

message("Wrote Pfam preparation outputs to: ", outdir)
