...
usage <- paste(
    "Usage:",
    "Rscript run_iupred2a_prepare.R",
...
features$is_iupred2a_candidate <- !is.na(features$isoform_switch_q_value) &
...
candidate_table <- features[features$is_iupred2a_candidate | features$is_target_gene, , drop = FALSE]
...
output_prefix <- "isoform_iupred2a_candidates"
...
notes <- c(
    ...
    sprintf("Significant switch candidates in feature table: %d", sum(features$is_iupred2a_candidate, na.rm = TRUE)),
    ...
    "",
    "Outputs:",
    "- isoform_iupred2a_candidates_AA.fasta: amino-acid FASTA for IUPred2A/ANCHOR2.",
    "- iupred2a_top_candidates.csv: compact table of strongest isoforms.",
...
writeLines(notes, file.path(outdir, "iupred2a_prepare_notes.txt"))

message("Wrote IUPred2A preparation outputs to: ", outdir)
