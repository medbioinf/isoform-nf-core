...
usage <- paste(
    "Usage:",
    "Rscript run_deeptmhmm_prepare.R",
...
features$is_deeptmhmm_candidate <- !is.na(features$isoform_switch_q_value) &
...
candidate_table <- features[features$is_deeptmhmm_candidate | features$is_target_gene, , drop = FALSE]
...
output_prefix <- "isoform_deeptmhmm_candidates"
...
notes <- c(
    ...
    sprintf("Significant switch candidates in feature table: %d", sum(features$is_deeptmhmm_candidate, na.rm = TRUE)),
    ...
    "",
    "Outputs:",
    "- isoform_deeptmhmm_candidates_AA.fasta: amino-acid FASTA for DeepTMHMM.",
    "- deeptmhmm_top_candidates.csv: compact table of strongest isoforms.",
...
writeLines(notes, file.path(outdir, "deeptmhmm_prepare_notes.txt"))

message("Wrote DeepTMHMM preparation outputs to: ", outdir)
